...
try {
  ...
  // Buat database "NativeStockManager" (jika belum ada)
  $query = "CREATE DATABASE IF NOT EXISTS NativeStockManager ";
  $mysqli->query($query);
  if ($mysqli->error){
    throw new Exception($mysqli->error, $mysqli->errno);
  }
  else {
    echo "Database 'NativeStockManager' berhasil di buat / sudah tersedia <br>";
  };

  // Pilih database "NativeStockManager"
  $mysqli->select_db("NativeStockManager");
  if ($mysqli->error){
    throw new Exception($mysqli->error, $mysqli->errno);
  }
  else {
    echo "Database 'NativeStockManager' berhasil di pilih <br>";
  };
